...
@router.get("/search-image-caption/")
def search_images_caption(
    department: str = Query(None, description="Department name (optional)"),
    query: str = Query(..., description="Query string related to the image" ),
):
    # Use 5 Chunks of text to do the similarity search
    results = imagetotext_collection.query(
        query_texts=[query],
        n_results=5,
    )
...
@router.get("/search-image-embeddings/")
def search_images_embeddings(
    department: str = Query(None, description="Department name (optional)"),
    query: str = Query(..., description="Query using an image file path to find similar images to the image"),
):
    # Use 5 Chunks of text to do the similarity search
    embeddings = ImageEmbedding.get_image_embeddings(query)
    results = imagesembedding_collection.query(
